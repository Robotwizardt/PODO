...
{
    internal readonly record struct TrayMenuItem(uint CommandId, string Label);

    private const string SingleInstanceMutexName = @"Local\PODO.SingleInstance";
    private const string SingleInstancePipeName = "PODO.SingleInstance";
    ...
    protected override async void OnStartup(StartupEventArgs e)
    {
        ...
        try
        {
            ...
            var paperTodoHost = new PaperTodoHost(
                ...
                logger);
            await paperTodoHost.InitializeAsync(drawerService, todoService);
            ...
            mainViewModel.ProjectManagement.ProjectChanged += async (_, boxId) =>
                await GuardRefreshAsync(() => _desktopBoxManager.RefreshItemsAsync(boxId), "RefreshProjectBoxAsync", logger);
    ...
    private void InitializeTaskbarIcon(AppPaths paths, IAppLogger logger)
    {
        ...
        _taskbarIcon = new TaskbarIcon(nint.Zero, iconPath, "PODO");
        ...
        _taskbarIcon.RightClick += (_, _) =>
        {
            ...
            foreach (var item in CreateTrayMenuItems(_mainWindow.IsVisible))
            {
                AppendMenuW(menu, 0, item.CommandId, item.Label);
            }
        ...
        _taskbarIcon.MenuCommand += async (_, e) =>
        {
            switch (e.CommandId)
            {
                ...
                case 2:
                    ...
                    _paperTodoHost?.ShowAllPapers();
                    break;
                case 3:
                    _mainWindow?.OpenCreateBoxMenu();
                    break;
                case 4:
                    _mainWindow?.OpenDesktopPaperManager();
                    break;
                case 5:
    ...
    internal static IReadOnlyList<TrayMenuItem> CreateTrayMenuItems(bool isMainWindowVisible)
    {
        return
        [
            new TrayMenuItem(1, isMainWindowVisible ? "隐藏主窗口" : "显示主窗口"),
            new TrayMenuItem(2, "显示全部桌面内容"),
            new TrayMenuItem(3, "新建收纳盒"),
            new TrayMenuItem(4, "管理桌面便签"),
            new TrayMenuItem(5, "桌面便签设置"),
            new TrayMenuItem(6, "退出 PODO")
        ];
    }
    ...
    {
        var tempDir = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "PODO");
