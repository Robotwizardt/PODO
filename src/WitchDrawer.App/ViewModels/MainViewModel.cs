...
public sealed class MainViewModel : ObservableObject
{
    ...
    private const string StartupRegistryKeyName = "PODO";
    ...
    private int _externalBoxCollectionReloadQueued;
        ...
        FileDialogAccessSettingsStore? fileDialogAccessSettings = null)
    {
        ...
        CreateTodoBoxCommand = new AsyncRelayCommand(CreateTodoPaperAsync);
        CreateNoteBoxCommand = new AsyncRelayCommand(CreateNotePaperAsync);
        CreateProjectBoxCommand = new AsyncRelayCommand(() => CreateBoxAsync(BoxType.Project));
        ...
        ArchiveSelectedProjectCommand = new AsyncRelayCommand(
            ArchiveSelectedProjectAsync,
            () => SelectedBox?.IsProjectBox == true && !IsBusy);
    ...
    public IAsyncRelayCommand CreateNoteBoxCommand { get; }

    public IAsyncRelayCommand CreateProjectBoxCommand { get; }
    ...
    public IAsyncRelayCommand ShowProjectCommand { get; }
    ...
    public bool IsSelectedNoteBox => SelectedBox?.IsNoteBox == true;

    public bool IsSelectedProjectBox => SelectedBox?.IsProjectBox == true;

    public bool CanImportFiles => SelectedBox is { IsTodoBox: false, IsNoteBox: false, IsProjectBox: false };
    ...
    public async Task ImportPathsAsync(IEnumerable<string> paths)
    {
        ...
        {
            StatusText = "桌面便签请使用任务输入框添加事项";
            return;
        }

        if (selectedBox.IsNoteBox)
        {
            StatusText = "笔记便签请直接在桌面正文区域编辑";
            return;
        }

        if (selectedBox.IsProjectBox)
        {
            StatusText = "项目收纳盒请使用模块清单添加项目模块";
            return;
        }
    ...
    private void QueueExternalBoxCollectionReload()
    {
        if (Interlocked.Exchange(ref _externalBoxCollectionReloadQueued, 1) != 0)
        {
            return;
        }

        _ = ReloadBoxCollectionFromDesktopAsync();
    }

    private async Task ReloadBoxCollectionFromDesktopAsync()
    {
        try
        {
            // A desktop action may happen while the manager is saving another change.
            // Wait briefly instead of dropping the refresh and leaving the two surfaces stale.
            for (var attempt = 0; IsBusy && attempt < 20; attempt++)
            {
                await Task.Delay(50);
            }
        ...
        {
            _logger.Error(exception, "Failed to reload boxes after a desktop box action.");
        }
        finally
        {
            Interlocked.Exchange(ref _externalBoxCollectionReloadQueued, 0);
        }
    }

    private Task CreateTodoPaperAsync()
    {
        _paperTodoHost?.CreateTodoPaper();
        return Task.CompletedTask;
    }

    private Task CreateNotePaperAsync()
    {
        _paperTodoHost?.CreateNotePaper();
        return Task.CompletedTask;
    }

    private async Task ShowProjectAsync()
    {
        var projectBox = SelectedBox?.IsProjectBox == true
            ? SelectedBox
            : Boxes.FirstOrDefault(box => box.IsProjectBox);
        if (projectBox is null)
        {
            StatusText = "请先从左下角新建一个项目收纳盒";
            return;
        }
        ...
        IsSettingsPage = false;
        IsAboutPage = false;
        await SelectBoxAsync(projectBox);
    }
    ...
    public async Task CreateBoundBoxAsync(string folderPath)
    {
        ...
        await RunBusyAsync(async () =>
        {
            ...
            var name = string.IsNullOrWhiteSpace(folderName) ? "目标收纳盒" : folderName;
            var box = await _drawerService.CreateBoundBoxAsync(name, fullFolderPath);
            var viewModel = new BoxViewModel(
                box,
                _drawerService,
            ...
            Boxes.Add(viewModel);
            await SelectBoxAsync(viewModel);
            StatusText = $"已绑定文件夹“{name}”，桌面目标收纳盒已生成";
            BoxesChanged?.Invoke(this, EventArgs.Empty);
        });
    }
        ...
        IReadOnlyList<Box> boxes)
    {
        return await Task.WhenAll(
            boxes.Where(box => box.Type != BoxType.ProjectFolder).Select(async box =>
            {
    ...
    private bool UpdateSelectedBoxCore(BoxViewModel? value)
    {
        ...
        OnPropertyChanged(nameof(IsSelectedProjectBox));
        ...
        CancellationToken cancellationToken)
    {
        if (selectedBox?.IsTodoBox == true)
        {
            ...
            await ProjectManagement.LoadAsync(null, null);
            return;
        }

        if (selectedBox?.IsNoteBox == true)
        {
            if (IsCurrentItemsLoad(selectedBox, version))
            {
                Items.ReplaceAll([]);
        ...
        if (selectedBox?.IsProjectBox == true)
        {
            if (IsCurrentItemsLoad(selectedBox, version))
            {
                Items.ReplaceAll([]);
            ...
            await TodoBoxDetail.LoadAsync(null);
            await ProjectManagement.LoadAsync(selectedBox.Id, selectedBox.Name);
            return;
        }
    ...
    private async Task ArchiveSelectedProjectAsync()
    {
        ...
        await RunBusyAsync(async () =>
        {
            var snapshot = await _projectService.GetProjectArchiveSnapshotAsync(selectedBox.Id);
            var paperIds = snapshot.LinkedPapers.Select(link => link.PaperId).ToArray();
            var archivedPaperIds = _paperTodoHost?.ArchivePapers(paperIds) ?? [];

            try
            {
                await _projectService.ArchiveProjectAsync(selectedBox.Id);
            }
            catch
            {
                _paperTodoHost?.RestoreArchivedPapers(archivedPaperIds);
                throw;
            }

            await ReloadVisibleBoxesAsync(selectFirstWhenMissing: false);
            await _quickPanelViewModel.LoadAsync();
            IsArchivePage = true;
            IsSettingsPage = false;
            IsAboutPage = false;
    ...
    private async Task RestoreArchivedProjectAsync(ArchivedProjectViewModel? project)
    {
        if (project is null)
        {
            return;
        ...
        await RunBusyAsync(async () =>
        {
            var snapshot = await _projectService.RestoreProjectAsync(project.Id);
            try
            {
                _paperTodoHost?.RestoreArchivedPapers(
            ...
            await ReloadVisibleBoxesAsync(snapshot.ProjectBox.Id);
            await _quickPanelViewModel.LoadAsync();
            await LoadArchivedTodosAsync();
            await LoadArchivedProjectsAsync();
            LoadArchivedPapers();
    ...
    private async Task LoadArchivedTodosAsync()
    {
        try
        {
            ...
            {
                var boxName = boxNames.GetValueOrDefault(todo.BoxId, "桌面便签");
    ...
    private async Task LoadArchivedProjectsAsync()
    {
        try
        {
            var archivedProjects = (await _drawerService.GetArchivedBoxesAsync())
                .Where(box => box.Type == BoxType.Project)
                .ToArray();
            var archivedPaperTitles = (_paperTodoHost?.GetArchivedPapers() ?? [])
                .GroupBy(paper => paper.Id, StringComparer.Ordinal)
                .ToDictionary(
                    group => group.Key,
            ...
            foreach (var project in archivedProjects)
            {
                ...
                var linkedBoxNames = snapshot.LinkedBoxes
                    ...
                    .ToArray();
                var linkedPaperTitles = snapshot.LinkedPapers
                    .Select(link => archivedPaperTitles.GetValueOrDefault(link.PaperId, "桌面便签"))
                    .ToArray();
                projectViewModels.Add(
                    new ArchivedProjectViewModel(project, linkedBoxNames, linkedPaperTitles));
            }
    ...
    private async Task ReloadVisibleBoxesAsync(
        ...
        bool selectFirstWhenMissing = true)
    {
        ...
        var preferredSelection = preferredSelectionId is Guid boxId
            ? Boxes.FirstOrDefault(box => box.Id == boxId)
            : null;
        await SelectBoxAsync(preferredSelection ?? (selectFirstWhenMissing ? Boxes.FirstOrDefault() : null));
    }
