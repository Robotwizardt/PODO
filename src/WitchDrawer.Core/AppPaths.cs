...
/// <summary>
/// 应用本地数据路径约定。
/// 默认使用 %LocalAppData%\PODO，可通过环境变量覆盖。
/// </summary>
public sealed record AppPaths(string RootDirectory)
{
    /// <summary>
    /// 用于覆盖默认数据根目录的环境变量名。
    /// 例如：PODO_DATA_DIR=D:\data\PODO
    /// </summary>
    public const string DataDirectoryEnvironmentVariableName = "PODO_DATA_DIR";

    /// <summary>
    /// 默认数据目录名（位于 LocalApplicationData 下）。
    /// </summary>
    public const string DefaultRootDirectoryName = "PODO";

    /// <summary>
    /// 数据库文件名。
    /// </summary>
    public const string DatabaseFileName = "podo.db";
    ...
    private void EnsureRootDirectoryWritable()
    {
        ...
        {
            throw new InvalidOperationException(
                "PODO 数据目录不可写，SQLite 无法创建数据库旁路文件（-wal/-shm）。"
                + Environment.NewLine
                + "数据目录: "
                + RootDirectory
                + Environment.NewLine
