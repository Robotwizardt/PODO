...
public sealed class BoxSizeSettingsTests
{
    ...
    [Fact]
    public async Task SizeSettingsViewModel_ProjectBoxCanPersistCustomSize()
    {
        var root = CreateTempRoot();
        try
        {
            ...
            viewModel.SetTargetBox(
                new BoxViewModel(projectBox, drawerService, BoxVisualStyle.Modern, false));
            await Task.Delay(200);

            Assert.True(viewModel.IsProjectBox);
            Assert.Equal("项目尺寸", viewModel.SizeControlLabel);
            Assert.Equal("紧凑", viewModel.AdaptiveModeLabel);
            Assert.Equal("自定义", viewModel.FixedModeLabel);

            await viewModel.UseFixedModeCommand.ExecuteAsync(null);
            var startingWidth = viewModel.FixedColumns;
            await viewModel.IncreaseColumnsCommand.ExecuteAsync(null);

            Assert.True(viewModel.IsFixedMode);
            Assert.Equal(startingWidth + 1, viewModel.FixedColumns);
            Assert.True(
                BoxSizeModeState.Parse(
                    await drawerService.GetSettingAsync(
                        BoxViewModel.GetSizeModeSettingKey(projectBox.Id))).IsFixed);
        }
        finally
        {
            CleanupTempRoot(root);
    ...
    [Fact]
    public async Task SizeSettingsViewModel_AllowsViewportSmallerThanOccupiedExtent()
    {
        var root = CreateTempRoot();
        try
        {
            ...
            // “海城项目文件夹”的实际情形：10 项横向占用 10 × 1，
            // 但固定规格是视口，不应把内容范围当作缩小下限。
            WeakReferenceMessenger.Default.Send(new BoxGridExtentChangedMessage(box.Id, 10, 1));

            Assert.Equal((10, 1), viewModel.SelectedExtent);
            Assert.True(viewModel.IsFixedMode);
            Assert.Equal(10, viewModel.FixedColumns);

            for (var attempt = 0; attempt < 9; attempt++)
            {
                await viewModel.DecreaseColumnsCommand.ExecuteAsync(null);
            }

            Assert.Equal(1, viewModel.FixedColumns);
    ...
    [Fact]
    public async Task SizeSettingsViewModel_AcceptsConcurrentGridExtentNotifications()
    {
        var root = CreateTempRoot();
        try
        {
            ...
            await Task.WhenAll(Enumerable.Range(1, 128).Select(extent =>
                Task.Run(() => WeakReferenceMessenger.Default.Send(
                    new BoxGridExtentChangedMessage(box.Id, extent, 2)))));

            Assert.InRange(viewModel.SelectedExtent.Columns, 1, 128);
            Assert.Equal(2, viewModel.SelectedExtent.Rows);
        }
        finally
        {
            CleanupTempRoot(root);
    ...
    [Fact]
    public async Task DesktopBoxSizeCommands_EnableFixedModeResizeAndPersistIt()
    {
        var root = CreateTempRoot();
        try
        {
            ...
            await viewModel.UseFixedSizeCommand.ExecuteAsync(null);
            await viewModel.IncreaseFixedColumnsCommand.ExecuteAsync(null);
            await viewModel.DecreaseFixedRowsCommand.ExecuteAsync(null);

            Assert.True(viewModel.IsFixedSize);
            Assert.Equal(5, viewModel.SizeMode.Columns);
            Assert.Equal(3, viewModel.SizeMode.Rows);
            Assert.Equal(
                "Fixed:5:3",
                await drawerService.GetSettingAsync(BoxViewModel.GetSizeModeSettingKey(box.Id)));

            await viewModel.UseAdaptiveSizeCommand.ExecuteAsync(null);
            Assert.False(viewModel.IsFixedSize);
            Assert.Equal(
                "Adaptive",
                await drawerService.GetSettingAsync(BoxViewModel.GetSizeModeSettingKey(box.Id)));
...
public sealed class FixedModeImportEnforcementTests
{
    ...
    [Fact]
    public async Task FixedBox_OneByOneViewportKeepsTenOverflowItemsDistinctAndScrollable()
    {
        var root = CreateTempRoot();
        try
        {
            ...
            var viewModel = new DesktopBoxViewModel(
                ...
                new RecordingLogger(),
                BoxVisualStyle.Modern);
            viewModel.ApplySizeMode(new BoxSizeModeState(true, 1, 1));
            await viewModel.LoadAsync();
            ...
            Assert.Equal(
                ...
                viewModel.Items
                    ...
                    .Distinct()
                    .Count());
            Assert.All(viewModel.Items, item => Assert.Equal(0, item.GridColumn));
            Assert.Equal(9, viewModel.Items.Max(item => item.GridRow));
            Assert.True(viewModel.GridCanvasHeight > viewModel.GridViewportHeight);
            Assert.Equal(
