...
{
    [Fact]
    public async Task EnsureLoadedAsync_DoesNotResetAnAlreadyLoadedFileList()
    {
        var root = CreateTempRoot();
        try
        {
            var (drawerService, repository) = await CreateDrawerServiceAsync(root);
            var box = await drawerService.CreateBoxAsync("关联收纳盒", BoxType.Normal);
            await drawerService.ImportPathAsync(
                box.Id,
                CreateSourceFile(root, "linked-item.txt"));
