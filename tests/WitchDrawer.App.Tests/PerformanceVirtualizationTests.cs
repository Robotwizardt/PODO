...
{
    [Fact]
    public void OpeningProjectFolderMember_RefreshesOnlyTheOpenedProjectsAttachments()
    {
        var openedProjectId = Guid.NewGuid();
        var otherProjectId = Guid.NewGuid();
        var now = DateTimeOffset.UtcNow;
        var projects = new[]
        {
            new Box(openedProjectId, "Opened", BoxType.Project, null, 0, now, now),
            new Box(otherProjectId, "Other", BoxType.Project, null, 1, now, now)
        };

        var refreshScope = ProjectAttachmentLayoutScope.ForProject(openedProjectId);

        Assert.Equal(
            [openedProjectId],
            projects.Where(refreshScope.Includes).Select(project => project.Id));
    }
